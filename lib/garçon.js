...
Framework.prototype.scanFiles = function(callback) {
  var Scanner = function(framework, callback) {
    ...
    that.pathsToExclude = /(^\.|\/\.|tmp\/|debug\/|test_suites\/|setup_body_class_names)/;
    ...
    if (htmlStylesheets === undefined) {
      that.frameworks.forEach(function(framework) {
        for (url in that.server.files) {
          ...
          if (file.framework === framework && file.isStylesheet()) {
            html.push('<link href="' + that.urlPrefix + url + '" rel="stylesheet" type="text/css">');
          }
        }
      });
    } else {
        ...
        if (/bootstrap/.test(framework.path)) {
          html.push(
            '<script type="text/javascript">',
              'String.preferredLanguage = "' + that.buildLanguage + '";',
...
App.prototype.save = function(savePath) {
  var that = this,
      stylesheets = [],
      scripts = [],
      stylesheet, script, html;
  
  var Saver = function(file) {
    var that = this;
    ...
    that.save = function() {
      file.handler.handle(file, null, function(r) {
        ...
        if (r.data.length > 0) {
          path = l.path.join(savePath, file.savePath());
  ...
  if (!savePath) savePath = 'build';
  //that.urlPrefix = '/' + savePath + '/';
  that.urlPrefix = '../';
  ...
  that.frameworks.forEach(function(framework) {
    for (url in that.server.files) {
      ...
      if (file.framework === framework) {
        ...
        if (file.isResource()) new Saver(file).save();
      }
  ...
  that.handle = function(file, request, callback) {
    ...
    if (count === 0) {
      ...
      files.forEach(function(file, i) {
        var next = file.handler ? file.handler : that.next;
