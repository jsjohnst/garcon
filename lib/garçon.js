...
App.prototype.save = function(savePath) {
  ...
  for (path in this.server.files) {
    file = this.server.files[path];
    
    if (this.frameworks.indexOf(file.framework) !== -1 || file.framework === this) {
      if (file.isScript() || file.isStylesheet() || file.isResource() || file.isHtml) {
        new Saver(file).save();
      }
    }
  }
